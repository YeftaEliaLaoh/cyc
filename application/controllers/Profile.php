	...
	public function friends_page($userid) 
	{
		...
		foreach($friends->result() as $r) {
			if($user->ID == $this->user->info->ID) {
				$options = '<a href="'.site_url("profile/" . $r->username) . '" class="btn btn-success btn-xs">'.lang("ctn_659").'</a> ';
				$options .= '<a href="'.site_url("profile/deadd/" . $r->ID . "/" . $this->security->get_csrf_hash()).'" class="btn btn-danger btn-xs">'.lang("ctn_57").'</a>';
			} else {
				$options = "";
			}
			$this->datatables->data[] = array(
