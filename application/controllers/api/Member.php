...
class Member extends REST_Controller
{
                        ...
                        if (!ctype_alnum($pass)){
                            ...
                            if ($login->num_rows() == 0) {
                                ...
                                $details = array(
                                    ...
                                    "themes"=>$r->themes_color,
                                    "province"=>$r->province,
                                    "city"=>$r->city,
                                    "personal_club"=>$r->personal_club,
                                    ...
                                    "phone"=>$r->phone,
                                    "youth_leader_name"=>$r->youth_leader_name,
                                    "senior_pastor_name"=>$r->senior_pastor_name,
    ...
    public function register2_post()
    {
        ...
        if(isset($code)) {
            if($code == $this->common->keycode()) {
                ...
                if($personal_club == "10") { // Personal
                ...
                } elseif ($personal_club == "11") { // Church
                    
                    $church_name = $this->common->nohtml(
                        $this->input->post("church_name", true));
                    $senior_pastor_name = $this->common->nohtml(
                        $this->input->post("senior_pastor_name", true));
                    $address = $this->common->nohtml(
                        $this->input->post("address", true));
                    $phone = $this->common->nohtml(
                        $this->input->post("phone", true));
                    $youth_leader_name = $this->common->nohtml(
                        $this->input->post("youth_leader_name", true));
                    $person_in_charge = $this->common->nohtml(
                        $this->input->post("person_in_charge", true));
                    $admin_phone = $this->common->nohtml(
                        $this->input->post("admin_phone", true));
                    ...
                    if ($status == 1) {
                        ...
                        $userid = $this->register_model->add_user(array(
                            ...
                            "senior_pastor_name" => $senior_pastor_name,
                            "address_1" => $address,
                            "youth_leader_name" => $youth_leader_name,
                ...
                } elseif ($personal_club == "12") { // Community
                    $community_name = $this->common->nohtml(
                        $this->input->post("community_name", true));
                    $phone = $this->common->nohtml(
                        $this->input->post("phone", true));
                    $person_in_charge = $this->common->nohtml(
                        $this->input->post("person_in_charge", true));
                ...
                $token =  $this->input->post('token');
                    ...
                    if(!empty($token)) {
                            ...
                            if($check->num_rows() > 0) {
                                ...
                                $total = $this->user_model->get_total_friends_count($a->ID);
                                $following = $this->user_model->get_total_following($a->ID);
                                $follower = $this->user_model->get_total_follower($a->ID);

                                $total_post = $this->feed_model->get_total_post($a->ID);

                                $c = $ch->row();
                                $c->profile_header = base_url() . $this->settings->info->upload_path_relative . "/" . $c->profile_header;
                                $c->avatar = base_url("uploads/".$c->avatar);
                                //$c->image_banner = base_url("uploads/".$c->image_banner);
                                if($c->premium_planid == 1) {
                                    $c->premium_planname = "Badge";
                                } else {
                                    $c->premium_planname = "User";
                                }
                                $c->post_count = $total_post;
                            ...
                            if($check->num_rows() > 0) {
                                ...
                                $interest = $this->common->nohtml(
                                    ...
                                    */
                                    ...
                                    /*
                                    if (!$this->register_model->check_username_is_free($username) && $ch == 2) {
                                        $stat = -1;
                                        $msg = lang("error_16");
                                    }
                            
                                    if (!$this->register_model->checkEmailIsFree($email) && $ch == 1) {
                                        $stat = -1;
                                        $msg = lang("error_20");
                                    }
                                    */
                                    ...
                                    if ($status == 1) {
                                        ...
                                        $this->user_model->update_user($a->ID, $update_data);
                                    }
                            } else {
                                $status = -2;
                                $msg = lang("error_85");
    ...
    public function edit_profile2_post()
    {
        $code = $this->input->post('cod3');
        if(isset($code)) {
            if($code == $this->common->keycode()) {
                                ...
                                $senior_pastor_name = $this->common->nohtml(
                                    ...
                                    */
                                    ...
                                    if ($status == 1) {
                                        ...
                                        if ($a->personal_club == "11") {
                                            $update_data["church_name"] = $first_name;
                                            $update_data["senior_pastor_name"] = $senior_pastor_name;
                                            $update_data["youth_leader_name"] = $youth_leader_name;
                                            $update_data["admin_phone"] = $admin_phone;
                                            $update_data["person_in_charge"] = $person_in_charge;
                                        } elseif ($a->personal_club == "12") {
                                            $update_data["community_name"] = $first_name;
                                            $update_data["person_in_charge"] = $person_in_charge;
                                        }
                                        $this->user_model->update_user($a->ID, $update_data);
                                    }
                ...
                $categoryid =  $this->input->post('category_id');
                    ...
                    if(!empty($token)) {
                            ...
                            if($check->num_rows() > 0) {
                                ...
                                } elseif(isset($_FILES['image_file']['size']) && count($_FILES['image_file']['name']) > 0) {
                                    ...
                                    if($cpt <= 10){
                                        ...
                                        for($i=0; $i<$cpt; $i++)
                                        {
                                            ...
                                            if ( !$up )
                                            {
                                                ...
                                                $data[] = $this->upload->data();
                                                $this->resizeImage($data[$i]['file_name']);
                                            }
                                        ...
                                        if(count($data) > 0) {
                                            for($j=0;$j<count($data);$j++){
                                                ...
                                                $idimage = $this->feed_model->add_image(array(
                                                    "file_name" => $data[$j]['file_name'],
                                                    "file_type" => $data[$j]['file_type'],
                                                    "extension" => $data[$j]['file_ext'],
                                                    "file_size" => $data[$j]['file_size'],
                                                    "userid" => $a->ID,
                                                    "timestamp" => time(),
                                                    "albumid" => $albumid
                                                    )
    ...
    public function loadfeedmember2_post()
    {
        $code = $this->input->post('cod3');
        $posts = array();
        if(isset($code)) {
                    ...
                    if(!empty($token)) {
                        $friendid = $userid;
                            ...
                            if($check->num_rows() > 0) {
                                ...
                                if($status == 1) {
                                    $post = $this->feed_model->get_load_feed_member1($friendid, $page,$limit,$filter)->result();
                                    foreach($post as $p) {
                                        if($p->premium_planid == 1) {
                                            $p->premium_planname = "Badge";
                                        } else {
                                        ...
                                        if(!empty($p->imageid)) {
                                            $arr_image = array();
                                            $idsImage = explode(",",$p->imageid);
                                            foreach($idsImage as $idImg) {
                                                // var_dump($idImg);
                                               $imageData = $this->feed_model->get_user_images($idImg);
                                            //    var_dump($imageData);
                                            //    {
                                            //         "ID": "206",
                                            //         "image_file_name": "http://dev.andtechnology.me/uploads/6787cdc44d56f8b041d1812bc515c0ac.jpeg",
                                            //         "image_file_url": ""
                                            //     },
                                               $arr_image[] = array(
                                                    "ID" => $imageData->ID,
                                                    "image_file_name" => base_url() . $this->settings->info->upload_path_relative . "/" . $imageData->file_name,
                                                    "image_file_url" => $imageData->file_url
                                               );
                                            }
                                            $p->images = $arr_image;
                                        }
    ...
    public function loadfeedtag_post()
    {
        $code = $this->input->post('cod3');
        $posts = array();
        if(isset($code)) {
                    ...
                    if(!empty($token)) {
                        $friendid = $userid;
                            ...
                            if($check->num_rows() > 0) {
                                ...
                                if($status == 1) {
                                    $post = $this->feed_model->get_load_feed_tag_member($userid, $page,$limit,$filter);
                                    if(!empty($post)) {
    ...
    public function list_detail_story_post()
    {
        $code = $this->input->post('cod3');
        $posts = array();
        if(isset($code)) {
                    ...
                    if(!empty($token)) {
                        $friendid = $userid;
                            ...
                            if($check->num_rows() > 0) {
                                ...
                                if($status == 1) {
                                    $post = $this->feed_model->get_load_story($userid, $page,$limit)->result();
                                    foreach($post as $p) {
    ...
    public function reply_story_post()
    {
                ...
                $id =  $this->input->post('id');
                    ...
                    if(!empty($token)) {
                            ...
                            if($check->num_rows() > 0) {
                                ...
                                $c2 = $this->user_model->get_friend_story($id)->row();
                                $friendid = $c2->userid;
                ...
                $token =  $this->input->post('token');
                    ...
                    if(!empty($token)) {
                            ...
                            if($check->num_rows() > 0) {
                               ...
                               $post = $this->feed_model->get_home_feed_new($user, $page,$limit,$category);
                                //print_r($this->db->last_query());  
                                //$post = $post->result();
                                foreach($post as $r) {
                                    $r->avatar = base_url() . $this->settings->info->upload_path_relative . "/" . $r->avatar;
                                    ...
                                    if(!empty($r->images)) {
                                        foreach($r->images as $image) {
                                            if(!empty($image->image_file_name)) {
                                                $image->image_file_name = base_url() . $this->settings->info->upload_path_relative . "/" . $image->image_file_name;
                                            }
                                        }
                                    }
        ...
        if(isset($code)) {
                ...
                $data = array();
                    ...
                    if(!empty($token)) {
                            ...
                            if($check->num_rows() > 0) {
                                ...
                                // $post = $this->feed_model->get_post($id,$user->ID);
                                $post = $this->feed_model->get_detail_post($id);
                                if($post->num_rows() == 0) {
                                    $msg =  lang("error_105");
                                }
            ...
            if($data['like_status']){
                if($post->userid > 0 && $post->userid != $user->ID) {
                    ...
                    // var_dump("masuk notif");
                }
            }
            // var_dump($data['like_status']);
...
*/ 
    ...
    public function get_detailpost2_post() 
	{
        $code = $this->input->post('cod3');
        $list = array();
        if(isset($code)) {
                ...
                $id = $this->input->post('id');
                    ...
                    if(!empty($token)) {
                            ...
                            if($check->num_rows() > 0) {
                                ...
                                $list->images = array();
                                if(!empty($list->imageid)) {
                                    $arr_image = array();
                                    $idsImage = explode(",",$list->imageid);
                                    foreach($idsImage as $idImg) {
                                       $imageData = $this->feed_model->get_user_images($idImg);
                                    //    {
                                    //         "ID": "206",
        ...
        if(isset($code)) {
            if($code == $this->common->keycode()) {
                ...
                if($check->num_rows() > 0) {
                    ...
                    $msg = "success";
                    $user = $check->row();
                    $this->db->query("UPDATE users SET token = '', device_id = '' WHERE ID = $user->ID");
                }
