...
class Login_Model extends CI_Model 
{
	...
	public function getUserByEmail($email) 
	{
		...
		*/
		return $this->db->query("SELECT u.*,t.color as themes_color,p.name as province_name, c.name as city_name FROM users u LEFT JOIN themes t ON t.id=u.themes 
		LEFT JOIN provinces p ON p.id=u.province
	...
	public function getUserByUsername($username) 
	{
		...
		*/
		return $this->db->query("SELECT u.*,t.color as themes_color FROM users u LEFT JOIN themes t ON t.id=u.themes WHERE u.username = '$username'");
	}
	...
	{
		$this->db->where("ID", $userid)
		->update("users", array("device_id" => $deviceid));
	}
