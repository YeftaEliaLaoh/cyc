var DZ = {
  ...
  initDataTable: function (data) {
    ...
    data.initComplete = data.initComplete || function(settings, json) {
      DZ.unblockUI($(tableId));
    };
    data.drawCallback = data.drawCallback || function () {
      DZ.unblockUI($(tableId));
    };
    ...
    return $(tableId).DataTable( {
      ...
      initComplete: data.initComplete,
      drawCallback: data.drawCallback
    } );
  },
  initDataTableModel: function (data, e) {
    var tableId = data.tableId;

    DZ.blockUI($(tableId, e));
    var ret = {};
    ret.url = DZ.get['serviceUri'] + data.url;
    ret.data = data.data || {};
    ret.type = data.type ? 'get' : 'post';
    data.initComplete = data.initComplete || function(settings, json) {
      DZ.unblockUI($(tableId, e));
    };
    data.drawCallback = data.drawCallback || function () {
      DZ.unblockUI($(tableId, e));
    };
    $(tableId, e).DataTable().destroy();
    return $(tableId, e).DataTable( {
      ordering: true,
      processing: true,
      serverSide: true,
      lengthMenu: [[20, 50, -1], [20, 50, "All"]],
  ...
  initDataTableFixedColoum: function (data) {
    var tableId = data.tableId;
    ...
    return $(tableId).DataTable( {
      ...
      initComplete: data.initComplete,
          drawCallback: function () { // this gets rid of duplicate headers
            $('.dataTables_scrollBody thead').css({ "display": "table-footer-group" });
            $('.DTFC_LeftBodyLiner').css("overflow-y", "");
            $(tableId+'>thead>tr>th').css("backgroundColor", "");
          },
        } );
  },
...
ajax_val: function (xhr_req) {
  if(xhr_req != null && xhr_req && xhr_req.readyState != 4){
    if(xhr_req != null) {
     xhr_req.abort();
     // xhr_req = null;
   }
...
showAlertWarning: function (msg){
  ...
  $('html').scrollTop($('.page-title').offset().top);
  $('html').scrollLeft($('.page-title').offset().left);
  setTimeout(function(){
    $(".alert").remove();
  },10000);
},
...
getDefaultOptionMultiple: function (url_name, view_id, e, status_default ,default_text, datas) {
...
getDefaultOptionGroupMultiple2: function (url_name, view_id, e, status_default ,default_text, datas) {
  $(view_id, e).empty();
  $(view_id, e).select2({
...
getDefaultOptionGroup: function (url_name, view_id, e, status_default ,default_text, datas) {
  $(view_id, e).empty();
  $(view_id, e).select2();
  return DZ.ajax({
    url: url_name,
...
formatPrice: function (num) {
  ...
  var number =Number.parseFloat(num).toFixed(2).replace(/./g, function(c, i, a) {
